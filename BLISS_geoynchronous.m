...
% NOTE: Editied version of the full trajectory optimization script
...
Rot         = @(a) [cos(a) sin(a) 0;-sin(a) cos(a) 0;0 0 1];   
% x0_NEO = [m_AU+100*rGEO;0;0;0;-1.3*sqrt(mu_Sun/(m_AU+100*rGEO));0];
...
if Plot_inputs
    tStart = tic;
    numt = numel(tSim);
    [xdotSim,avec_Sun_EarthSim,avec_Sun_SailSim,avec_Earth_SailSim,avec_SRP_SailSim] = cellfun(@(t,x) fsim(t,x,mu_Sun,mu_Earth,m,P_SRP,C_SRP,A_SRP), num2cell(tSim), num2cell(xSim',1)','uni',0);
